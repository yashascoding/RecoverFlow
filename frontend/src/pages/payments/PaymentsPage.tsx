  ...
  const columns: Column<Payment>[] = [
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="font-mono text-[12px]">{truncateId(row.razorpay_order_id, 12)}</span>
      ),
    },
    {
      key: 'customer_name',
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="text-muted-foreground whitespace-nowrap">{formatDateTime(row.created_at)}</span>
      ),
    },
    {
      key: 'recovery_status',
      header: 'Recovery',
      render: (row) => row.recovery_status ? <StatusBadge status={row.recovery_status as never} /> : <span className="text-muted-foreground">—</span>,
    },
  ]
